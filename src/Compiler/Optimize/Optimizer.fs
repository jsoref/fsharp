...
and OptimizeExprOpReductionsAfter cenv env (op, tyargs, argsR, arginfos, m) =
    ...
    | Some value -> 
        match TryOptimizeVal cenv env (None, false, false, value, m) with 
        | Some res -> OptimizeExpr cenv env res (* discard e1 since guard ensures it has no effects *)
        | None -> OptimizeExprOpFallback cenv env (op, tyargs, argsR, m) arginfos value
    | None -> OptimizeExprOpFallback cenv env (op, tyargs, argsR, m) arginfos UnknownValue

and OptimizeExprOpFallback cenv env (op, tyargs, argsR, m) arginfos value =
    let g = cenv.g
    ...
    let cost, value = 
      ...
      | TOp.ExnConstr _ -> 2, value

      | TOp.Tuple tupInfo -> 
          let isStruct = evalTupInfoIsStruct tupInfo 
          if isStruct then 0, value 
      ...
      | TOp.Bytes bytes -> bytes.Length/10, value
      | TOp.UInt16s bytes -> bytes.Length/10, value
      ...
      | TOp.Recd (ctorInfo, tcref) ->
          ...
          let value = 
              ...
              | RecdExpr -> 
                   if argValues.Length <> finfos.Length then value 
...
and OptimizeNewDelegateExpr cenv env (lambdaId, vsl, body, remake) = 
    ...
    let value = CurriedLambdaValue (lambdaId, arities, bsize, exprR, tyOfExpr g exprR) 
