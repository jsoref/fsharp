...
type internal LegacyProjectWorkspaceMap
    ...
    let optionsAssociation =
        ConditionalWeakTable<IFSharpWorkspaceProjectContext, string[]>()

    let isPathWellFormed (path: string) =
        not (String.IsNullOrWhiteSpace path)
        && path |> Seq.forall (fun c -> not (Set.contains c invalidPathChars))
    ...
    do solution.AdviseSolutionEvents(this) |> ignore
    ...
    member this.SetupLegacyProjectFile(siteProvider: IProvideProjectSite) =
        let rec setup (site: IProjectSite) =
            ...
            let hierarchy =
                site.ProjectProvider |> Option.map (fun p -> p :?> IVsHierarchy) |> Option.toObj
